...
describe("Pagination", () => {
	...
	const getPreviousPageButton = () =>
		cy.getByIdFragment(
			"button",
			"routesTable-pagination-go-to-previous-page"
		)
	...
	const getGoToPageButton = (number: number) =>
		cy.getByIdFragment(
			"span",
			"routesTable-pagination-go-to-page-" + number
		)
	...
	const verifyPaginationButtonsExist = (
		...
		getPreviousPageButton().should(
			currentPage === 1 ? "not.exist" : "exist"
		)
	...
	context("Test pagination of the Studio Routes table", () => {
		it("should paginate and load more", () => {
			cy.visitRoute(
				getWorkspaceBasePath(appName, workspaceName) + "/routes"
			)
			...
			cy.getByIdFragment(
				"span",
				"routesTable-pagination-go-to-page-2"
			).click()
