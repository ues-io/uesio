  ...
  Object.entries(collectionMetadata.getAllFieldMetadata()).forEach(
    ([uesioFieldName, fieldMetadata]) => {
      ...
      if (fieldMetadata.externalName) {
        uesioFieldsByExternalName[fieldMetadata.externalName] = uesioFieldName
      }
    },
  )

  const getUesioItemFromExternalRecord = (record: Record<string, FieldValue>) =>
    Object.entries(record).reduce(
      (acc: Record<string, FieldValue>, [externalField, value]) => {
        ...
        const fieldMetadata = collectionMetadata.getFieldMetadata(uesioName)
      ...
      switch (localField) {
        case "priority": {
          ...
          if (operator === "GT") {
            ...
            return values?.some((value) => (itemValue as number) === value)
