...
describe("View Only Wires", () => {
  ...
  context("Route Navigation", () => {
    it("The wire record should not disappear", () => {
      ...
      cy.getWireState(getViewID(username, appName, viewName), "myViewOnlyWire")
        .its("data")
        .should((obj) => {
      ...
      cy.getByIdFragment("select", "view-only-two").should((select) => {
        ...
        expect(select.children("option").eq(0)).to.have.text("Select an option")
      ...
      cy.getByIdFragment("select", "media-sub-type").should((select) => {
        ...
        expect(select.children("option").eq(0)).to.have.text("")
        expect(select.children("optgroup").eq(0).attr("label")).to.equal(
          "Image Type",
        )
        expect(select.children("optgroup").children("option")).to.have.length(3)
        expect(
      ...
      cy.getWireState(getViewID(username, appName, viewName2), "viewOnlyWire")
        .its("data")
        .should((obj) => {
          expect(Object.keys(obj).length).to.equal(1)
      ...
      cy.getWireState(getViewID(username, appName, viewName2), "regularWire")
        .its("data")
        .should((obj) => {
          expect(Object.keys(obj).length).to.equal(1)
