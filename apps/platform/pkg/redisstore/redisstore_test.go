...
func TestRedisInvalidate(t *testing.T) {
	...
	setMembers := []string{"somekey_1", "somekey_2"}
	cacheKeys.SetVal(setMembers)
	setKey := "gocache_tag_tag1"

	client := NewMockRedisClientInterface(ctrl)
	client.EXPECT().SMembers(ctx, setKey).Return(cacheKeys)
	client.EXPECT().Del(ctx, setMembers[0], setMembers[1]).Return(&redis.IntCmd{})
	client.EXPECT().Del(ctx, setKey).Return(&redis.IntCmd{})
