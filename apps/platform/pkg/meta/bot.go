...
func getParamDef(p *BotParam) (typeOutput, importOutput string, err error) {
	...
	if err != nil {
		return "", "", exceptions.NewBadRequestException(fmt.Errorf("Could not generate type for parameter: %s: %w", p.Name, err.Error()))
	}
...
func (b *Bot) GenerateTypeDefinitions() (string, error) {
	...
	if b.Name == "" || b.Namespace == "" {
		return "", exceptions.NewBadRequestException(errors.New("Bot name and namespace must be provided to generate types"))
	}
	if b.Params == nil {
		return "", nil
	}
