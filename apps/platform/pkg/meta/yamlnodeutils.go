...
func validateMetadataName(name string, expectedName string) error {
	if expectedName != "" && name != expectedName {
		return exceptions.NewBadRequestException(fmt.Errorf("Metadata name does not match filename: %s, %s", name, expectedName))
	}
	if !IsValidMetadataName(name) {
		return exceptions.NewBadRequestException(fmt.Errorf("Failed metadata validation, can only contain lowercase characters a-z, the underscore character and the numerals 0-9: %s", name))
	}
	return nil
}
