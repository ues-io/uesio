...
func DownloadUserFile(w http.ResponseWriter, r *http.Request) {
	...
	if userFileID == "" {
		ctlutil.HandleError(w, exceptions.NewBadRequestException(errors.New("missing required query parameter: userfileid")))
		return
	}
	buf := &bytes.Buffer{}
...
func DownloadAttachment(w http.ResponseWriter, r *http.Request) {
	...
	if recordID == "" {
		ctlutil.HandleError(w, exceptions.NewBadRequestException(errors.New("missing required attachment recordid")))
		return
	}
	if path == "" {
		ctlutil.HandleError(w, exceptions.NewBadRequestException(errors.New("missing required attachment path")))
		return
	}
