...
func validateMetadataName(field *wire.FieldMetadata) ValidationFunc {
	return func(change *wire.ChangeItem) *exceptions.SaveException {
		...
		if err == nil && !meta.IsValidMetadataName(fmt.Sprintf("%v", val)) {
			return exceptions.NewSaveException(change.RecordKey, field.GetFullName(), fmt.Errorf("Field: %s failed metadata validation, can only contain lowercase characters a-z, the underscore character and the numerals 0-9", field.Label))
		}
		return nil
	}
}
