...
func ValidateRegex(field *wire.FieldMetadata) ValidationFunc {
	...
	if err != nil {
		return func(change *wire.ChangeItem) *wire.SaveError {
			return wire.NewSaveError(change.RecordKey, field.GetFullName(), "Regex for the field: "+field.Label+" is not valid")
		}
	}
	return func(change *wire.ChangeItem) *wire.SaveError {
		val, err2 := change.FieldChanges.GetField(field.GetFullName())
		if err2 == nil && !regex.MatchString(fmt.Sprintf("%v", val)) {
			return wire.NewSaveError(change.RecordKey, field.GetFullName(), "Field: "+field.Label+" does not follow the expected format pattern")
		}
		return nil
	}
}
