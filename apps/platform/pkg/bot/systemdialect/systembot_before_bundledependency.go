...
func runBundleDependencyBeforeSaveBot(request *wire.SaveOp, connection wire.Connection, session *sess.Session) error {
	return request.LoopChanges(func(change *wire.ChangeItem) error {
		...
		// If the bundle dependency's associated bundle's unique key does not have the "repository" at the end of it,
