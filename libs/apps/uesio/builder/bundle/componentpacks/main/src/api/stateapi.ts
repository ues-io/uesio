...
const getSelectedSlotIndex = (
  ...
  for (const part of parts) {
    if (part === WILDCARD_PATH_TOKEN) {
      const [index] = selectedPath.trimToSize(slotPath.size() + 1).popIndex()
  ...
  if (path) {
    ...
    for (const token of pathArray) {
      ...
      for (const slotDef of componentDef.slots) {
        ...
        const selectedSlotIndex = getSelectedSlotIndex(path, basePath, slotDef)
        ...
        if (
          ...
          const compDef = get(context, basePath) as definition.DefinitionMap

          const componentType = metadata.getKey(componentDef)

          const componentId =
            (compDef[COMPONENT_ID] as string) || styles.hash(basePath.localPath)

          const mergeContext = context.addRecordDataFrame(
            get(context, selectedSlotBasePath) as wire.PlainWireRecord,
            selectedSlotIndex,
          )
...
const getComponentDef = (componentType: string | undefined) =>
  componentType ? (getComponentType(componentType) as ComponentDef) : undefined
...
const walkViewComponents = (
  ...
  const viewDef: definition.ViewDefinition | undefined = get(context, viewPath)
  // Start traversing!
  if (viewDef?.components?.length) {
    const keepWalking = walkComponentsArray(viewDef.components, context, visit)
    if (!keepWalking) {
      return
    }
  }
  ...
  if (viewDef?.panels) {
    for (const panel of Object.values(viewDef.panels)) {
      ...
      if (panel?.actions?.length) {
        const keepWalking = walkComponentsArray(panel.actions, context, visit)
        if (!keepWalking) {
          return
        }
      }
...
const walkComponentsArray = (
  ...
  for (const component of components) {
    ...
    for (const slot of componentDef.slots) {
      ...
      const keepWalking = walkComponentsArray(slotComponents, context, visit)
      if (!keepWalking) return false
    }
  }
  return true
