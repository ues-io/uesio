	...
	properties?.forEach((property) => {
		...
		if (type === "KEY") {
			...
			if (key) {
				...
				value = getDefAtPath(def, path) as string
			}
			setter = (value: string) => changeKey(context, path, value)
		} else if (type === "WIRE") {
			value = getDefAtPath(def, propPath) as string
			...
			if (!value && property.defaultToContext) {
				value = getClosestWireInContext(def, path)
			}
			...
			setter = NoOp
			value = getDefAtPath(def, propPath) as Record<
				string,
			...
			setter = NoOp
			value = getDefAtPath(def, propPath) as wire.PlainWireRecord[]
			if (type === "SIGNALS") {
