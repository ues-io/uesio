...
type Props = {
  ...
  isSelected: (ctx: context.Context, path: FullPath, fieldId: string) => boolean
  onClose: () => void
...
const FieldPicker: definition.UtilityComponent<Props> = (props) => {
  ...
  if (
    ...
    displayFields = Object.values(parentFieldMetadata.getSubFields() || {}).map(
      (field) => new collection.Field(field, context),
    )
  } else {
