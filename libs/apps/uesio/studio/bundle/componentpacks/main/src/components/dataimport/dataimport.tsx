...
const DataImport: definition.UC<DataImportDefinition> = (props) => {
  ...
  const memoCsvFields = useMemo(() => uploaded.csvFields, [uploaded.csvFields])
  ...
  const collectionInstance = api.collection.useCollection(context, collectionId)
        ...
        collectionFields.map((fieldName, i) => {
          ...
          return (
            <ImportBodyItem
              ...
              removeMapping={() => {
                const { [fieldName]: remove, ...rest } = spec.mappings
                setSpec({
                  ...spec,
                  mappings: {
                    ...rest,
