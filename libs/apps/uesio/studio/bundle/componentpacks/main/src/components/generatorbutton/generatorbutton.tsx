...
const getGenValueForParam = (
  ...
  switch (def.type) {
    case "RECORD":
      return (
        record?.getFieldValue<string>(`${fieldKey}->${collection.ID_FIELD}`) ??
...
const getGenParamValues = (
  ...
  return Object.fromEntries(
    params.map((def) => [def.name, getGenValueForParam(def, context, record)]),
  )
}
...
const getLayoutFieldFromParamDef = (def: param.ParamDefinition) => ({
  "uesio/io.field": {
    ...
    "uesio.display": getDisplayConditionsFromBotParamConditions(def.conditions),
  },
})
