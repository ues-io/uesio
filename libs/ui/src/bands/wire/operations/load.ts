...
export default async (
  ...
  try {
    ...
    if (wireWithOnLoadError) {
      wires.forEach((w) =>
        errContext.getWire(w.name)?.handleEvent("onLoadError", errContext),
      )
    } else {
  ...
  const loadedResults = responseWires.map((wire, index) => {
    ...
    return {
      ...
      conditions: mergeConditions(originalWire.conditions, wire.conditions),
      original: { ...wire.data },
      isLoading: false,
      batchid: nanoid(),
      paramsHash,
  ...
  const allResults = loadedResults.concat(preloadedResults, invalidWiresResults)
