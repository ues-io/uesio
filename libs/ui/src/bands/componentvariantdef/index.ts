...
const updateYaml = (state: ComponentVariantDef, payload: YamlUpdatePayload) => {
...
/**
 * Clone a yaml node. Currently only tested with componentvariantdef components.
...
const saveAllDefs = (state: EntityState<ComponentVariantDef>) => {
	const componentvariantdefs = state.entities
	for (const defKey of Object.keys(componentvariantdefs)) {
		const defState = componentvariantdefs[defKey]
...
const cancelAllDefs = (state: EntityState<ComponentVariantDef>) => {
	const componentvariantdefs = state.entities
	for (const defKey of Object.keys(componentvariantdefs)) {
		const defState = componentvariantdefs[defKey]
...
const ComponentVariantDefSlice = createSlice({
	name: "componentvariantdef",
	...
	extraReducers: (builder) => {
		...
		builder.addCase(
			...
			(state, { payload }: PayloadAction<MoveDefinitionPayload>) => {
				...
				if (
					toType === "componentvariantdef" &&
					fromType === "componentvariantdef" &&
					toItem === fromItem
				) {
...
export default ComponentVariantDefSlice.reducer
