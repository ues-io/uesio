...
/*
  ...
  This is a prototype build script based on https://github.com/omar-azmi/experiments_ts/blob/main/monaco_esbuild4/build.ts
  that loads monaco-loader as a module rather than AMD.

  The reason to move to ESM for monaco-editor vs. AMD is that when monaco-editor is loaded via its AMD loader, it
  creates a `define.amd` in global scope.  This causes issues for any dependency that isn't pure ESM (e.g., fuzzysort,
  ajv, papaparse, etc.) because they have UMD wrappers and end up using AMD loader instead of module/ESM loader.  To
  reproduce this issue, using the code in `main` branch:
    1. build everything
    2. navigate directly to the "build" page of a view (make sure do to a browser "refresh" on the build page)
    3. go to a collection->manage data->import data
  This will cause an error when attempting to load papaparse because papaparse has UMD and it detects define.amd.  To
  workaround the AMD problem introduced by monaco-editor, we use esbuild `define` config option to ensure `define.amd`
  is always undefined within the generated bundle.  Note that if you open the "view" from studio (not the build page of
  the view), even though monaco-editor is loaded just like in "build" view, the problem does not occur if you go to
  import data.  The reason is that on "view" page, studio bundle is loaded first (before io which causes monaco-editor to
  load) so when papaparse is required, define.amd isn't defined yet.  When going to "build" directly, io is loaded first
  causing define.amd to be defined so then when studio is required, papaparse uses AMD loader.

  Monaco-editor has some limitations when using ESM (see https://github.com/microsoft/monaco-editor/issues/4614)
  specifically around bundling.  There are some documented solutions/plugins for webpack & Vite but not for esbuild.
  There is discussion of solving these limitations but unlikely to occur any time soon (see ).

  The code in this branch will successfully load monaco-editor via ESM.  The current known limitations are:
    1. There are issues with CSS styling that seem to be specific to the way that we CSS style in/around/on the
       editor itself.  In a simple scenario, everything works as expected (see ) so the CSS issues still need
       to be investigated/resolved.  For example, line numbers do not show up, arrow icons to expand/collapse
       functions to not appear, code lines are all over the place especially when scrolling, etc.
    2. There is a limitation with sourcemaps in the esbuild plugin that results in the sourcemaps being generated
       in /dist/ui/map folder instead of next to the js itself which causes the browser devtools not to be
       able to load the maps (see https://github.com/omar-azmi/experiments_ts/issues/1).
    3. We use outfile vs. outdir so we can name the bundle uesio.js so all the "extra" worker files that are generated carry the "uesio" name with them
       instead of "editor.worker." (see https://github.com/omar-azmi/experiments_ts/issues/2)
    4. The bundle size when bundling monaco-editor ballons to ~5MB when minified.
    5. We really only need monaco-editor in io bundle for codefield.tsx, however to avoid changing the way pack works
       with esbuild and/or having a direct esbuild step for io bundle vs. using pack, this branch puts monaco-editor
       in uesio.js so we take the overhead of the bundle size for all page loads instead of only when codefield.tsx
       is actually used.  This could be moved to io bundle directly but would require custom pack command and/or direct
       esbuild which isn't ideal at this stage.
*/
